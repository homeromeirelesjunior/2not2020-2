...
function selectionSort(vetor, fnComp) {
    ...
    function encontrarMenor(vetor, inicio) {
        ...
        for(let i = inicio + 1; i < vetor.length; i++) {
            ...
            if(! fnComp(vetor[i], vetor[posMenor])) posMenor = i    // Usado o ! (not) para garantir a execução da função
            comparacoes++
        }
        return posMenor
    }
    ...
    for (i = 0; i < vetor.length - 1; i++){
        ...
        if(fnComp(vetor[i], vetor[posMenor])) {    // Ordem dos argumentos invetida
